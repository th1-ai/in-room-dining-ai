...
You are working inside **In-Room Dining AI** ("The Butler") — Takes room-service and poolside orders in any language — from the QR card on the room desk, web chat, or the Voice AI line..
...
**Main workflow:** `workflows/10-take-orders.md` (`tools/run.py --once`).
Each new guest message goes through `extract_order` (what did they order,
in what language, any dietary note), then a fully deterministic pricing and
gating pass (`tools/menu_engine.py:resolve_order` — menu match, kitchen
ETA, sold-out check, dietary gate, upsell decision), then
`draft_confirmation` (the guest-facing reply and the kitchen's dietary
line). Neither model call ever decides a price or whether an order needs a
person — that is always plain Python.

**Sub-agents:** none folded into this repo.

**Kitchen status:** once an order fires (`tools/review.py send`), move it
forward with `python3 tools/kitchen.py board` and
`python3 tools/kitchen.py advance <ref>` — each advance pushes the guest a
status update. An order that has not fired has no kitchen status yet.

**What always needs a human** (never configurable off — see
`docs/safety.md`):

- Any dietary or allergy note, named or ambiguous, whatever language it is
  written in - named-allergen detection covers English, Spanish, French,
  German, Italian, Portuguese, Greek and Dutch; any other language still
  always escalates, just without the specific allergen tag (`docs/safety.md`).
- A requested item that is sold out today (`config/agent.yaml:
  kitchen.sold_out`).
...
Never approve an order with a dietary/allergy gate on trust — confirm with
the kitchen or the guest first, every time.
